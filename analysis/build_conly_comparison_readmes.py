...
def cell_page(suite, dim, budget, details) -> str:
    ...
    a("## Mann-Whitney U: C-only vs each opponent")
    a("")
    a(
    ...
    dsc = dsc_ordering(suite, dim, budget)
    if dsc is not None:
        ordering, path = dsc
    ...
    a(
        ...
        "schedule against the full portfolio; the component ablations "
...
def top_page(details, summary) -> str:
    lines = []
    a = lines.append
    a("# C-only positioning campaign")
    a("")
    a(
        "MSC-CMA-ES (C-only) - the C configuration run alone - against "
        "the full benchmark portfolio on the composition class of the "
        "eight design-envelope cells. The component ablations "
        "(NO-NBC, FIXED-PHI, NO-EXCLUSION, B-only) are documented in "
        "`ablations/`; NEA2+ has its own comparison in "
        "`related_comparisons/nea2plus/`."
    )
    a("")
    a(
        "Statistics: two-sided Mann-Whitney U on 51 raw terminal "
        "errors per function, Bonferroni within each (cell, opponent) "
        "family over the composition functions; DSC with the k = 8 "
        "portfolio on the composition scope where available."
    )
    a("")
    a("| Cell | Budget | Functions | C-only vs MSC-CMA-ES (MW-U) | C-only vs ARRDE (MW-U) | Page |")
    a("|:--|:--|:--|:--|:--|:--|")
    for suite, dim, budget in SETTINGS:
        ...
        def triple(opp):
            rr = [
                s for s in summary
                if s["suite"] == suite
                and int(s["dimension"]) == dim
                and int(s["budget"]) == budget
                and s["opponent"] == opp
            ]
            if not rr:
                return "-"
            s = rr[0]
            return (
                f"{s['conly_better']} / {s['opponent_better']} / "
                f"{s['not_significant']}"
            )
        link = f"{suite}/d{dim}/budget_{budget}/README.md"
        a(
            f"| {suite.upper()} D={dim} | {fmt_budget(budget)} | {m} | "
            f"{triple('MSC-CMA')} | {triple('ARRDE')} | [page]({link}) |"
        )
    a("")
    a(
        "Triples read: C-only better / opponent better / not "
        "significant, per composition function of the cell."
    )
    a("")
    return "\n".join(lines)
