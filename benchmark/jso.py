#!/usr/bin/env python3
"""
benchmark/jso.py — jSO (Brest, Maucec, Boskovic 2017) benchmark runner
using minionpy's C++ backend as reference implementation.

jSO is a SHADE-family variant derived from LSHADE; it ranked second in the
CEC2017 competition and is a strong baseline for moderate-budget,
higher-dimensional problems.

NOTE on relTol:
    minionpy.Minimizer has a global early-stop criterion `relTol` (default
    1e-4) that triggers when the relative function-value tolerance falls
    below the threshold. jSO has no built-in restart mechanism (unlike
    ARRDE/LSRTDE), so it gets stopped at the first plateau — typically
    around 38% of the budget at D=10. We pass relTol=0.0 to disable this
    behavior and force jSO to use the full evaluation budget, matching the
    intent of the original Brest 2017 specification.

Writes to experiments/<suite>/d<dim>/jSO-minionpy/maxevals_<N>/.

Usage
-----
    python benchmark/jso.py --suite cec2017 --dim 10 \\
...
from _common import (
    ALGO_JSO, ImprovementRecorder, build_outdir,
...
# Disable minionpy's global early-stop criterion. See module docstring.
_REL_TOL = 0.0
...
def _run_seed(suite, fnum, dim, maxevals, seed):
    """Run one jSO seed. Returns (seed, final_err, improvements)."""
...
def build_parser():
    p = argparse.ArgumentParser(
        description='jSO (minionpy C++) benchmark runner.',
...
def main():
    ...
    outdir = args.outdir or build_outdir(
        args.suite, args.dim, ALGO_JSO, maxevals)
    ...
    print_header(args.suite, args.dim, ALGO_JSO,
    ...
    params_record = {
        'cli_args':   vars(args),
        'jso_config': {
            'algo_source': 'minionpy.Minimizer(algo="jSO")',
            'relTol':      _REL_TOL,
        },
    }
