#!/usr/bin/env python3
"""
benchmark/arrde.py — ARRDE (Adaptive Restart Differential Evolution)
benchmark runner using minionpy's C++ backend as reference implementation.

Writes to experiments/<suite>/d<dim>/ARRDE-minionpy/maxevals_<N>/.
...
from _common import (
    ALGO_ARRDE, ImprovementRecorder, build_outdir,
...
def _run_seed(suite, fnum, dim, maxevals, seed):
    """Run one ARRDE seed. Returns (seed, final_err, improvements)."""
...
def build_parser():
    p = argparse.ArgumentParser(
        description='ARRDE (minionpy C++) benchmark runner.',
...
def main():
    ...
    outdir = args.outdir or build_outdir(
        args.suite, args.dim, ALGO_ARRDE, maxevals)
    ...
    print_header(args.suite, args.dim, ALGO_ARRDE,
    ...
    params_record = {
        'cli_args':     vars(args),
        'arrde_config': {
            'algo_source': 'minionpy.Minimizer(algo="ARRDE")',
        },
    }
