#!/usr/bin/env python3
"""
benchmark/j2020.py — j2020 (Brest, Maucec, Boskovic 2020) benchmark runner
using minionpy's C++ backend as reference implementation.

j2020 is a DE variant with dual-population architecture: a larger population
for global exploration and a smaller one for local refinement, with crowding
selection and periodic best-individual migration. It is derived from jDE100
(CEC2019 winner) and was designed for low-dimensional optimization with
very large evaluation budgets. Per the ARRDE v3 paper, j2020 wins CEC2022
D=10 S-score against ARRDE and wins the official CEC2019 100-Digit
Challenge score (87.251 vs ARRDE 83.000).

NOTE on relTol:
    minionpy.Minimizer has a global early-stop criterion `relTol` (default
    1e-4). j2020 has its own restart logic; we pass relTol=0.0 explicitly
    for configuration consistency across all DE wrappers.

Writes to experiments/<suite>/d<dim>/j2020-minionpy/maxevals_<N>/.

Usage
-----
    python benchmark/j2020.py --suite cec2020 --dim 10 \\
...
from _common import (
    ALGO_J2020, ImprovementRecorder, build_outdir,
...
def _run_seed(suite, fnum, dim, maxevals, seed):
    """Run one j2020 seed. Returns (seed, final_err, improvements)."""
...
def build_parser():
    p = argparse.ArgumentParser(
        description='j2020 (minionpy C++) benchmark runner.',
...
def main():
    ...
    outdir = args.outdir or build_outdir(
        args.suite, args.dim, ALGO_J2020, maxevals)
    ...
    print_header(args.suite, args.dim, ALGO_J2020,
    ...
    params_record = {
        'cli_args':     vars(args),
        'j2020_config': {
            'algo_source': 'minionpy.Minimizer(algo="j2020")',
            'relTol':      _REL_TOL,
        },
    }
