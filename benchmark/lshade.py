#!/usr/bin/env python3
"""
benchmark/lshade.py — LSHADE (Tanabe, Fukunaga 2014) benchmark runner
using minionpy's C++ backend as reference implementation.

LSHADE (Linear Population Size Reduction Success-History-based Adaptive
Differential Evolution) is the foundational adaptive-DE variant that won
the CEC2014 competition. It pairs SHADE's success-history-based adaptation
of F and CR with a linear population-size reduction schedule. Most modern
SHADE-family variants (jSO, LSHADE-cnEpSin, NL-SHADE-RSP, ARRDE) are
derived from LSHADE, so including it as a baseline grounds the comparison
in the canonical starting point of the lineage.

NOTE on relTol:
    minionpy.Minimizer has a global early-stop criterion `relTol` (default
    1e-4). LSHADE has no built-in restart mechanism, so it can in principle
    be affected by relTol similarly to jSO. We pass relTol=0.0 to disable
    this and force LSHADE to use the full evaluation budget, matching the
    original Tanabe-Fukunaga 2014 specification.

Writes to experiments/<suite>/d<dim>/LSHADE-minionpy/maxevals_<N>/.

Usage
-----
    python benchmark/lshade.py --suite cec2017 --dim 10 \\
...
from _common import (
    ALGO_LSHADE, ImprovementRecorder, build_outdir,
...
def _run_seed(suite, fnum, dim, maxevals, seed):
    """Run one LSHADE seed. Returns (seed, final_err, improvements)."""
...
def build_parser():
    p = argparse.ArgumentParser(
        description='LSHADE (minionpy C++) benchmark runner.',
...
def main():
    ...
    outdir = args.outdir or build_outdir(
        args.suite, args.dim, ALGO_LSHADE, maxevals)
    ...
    print_header(args.suite, args.dim, ALGO_LSHADE,
    ...
    params_record = {
        'cli_args':      vars(args),
        'lshade_config': {
            'algo_source': 'minionpy.Minimizer(algo="LSHADE")',
            'relTol':      _REL_TOL,
        },
    }
